...
//            2017-07-13       kw  fix Typo in FEATURE_BST_CUTOUT_AVAIALABLE --> FEATURE_BST_CUTOUT_AVAILABLE
//            2017-12-13 V0.24 ab  added local messages and constants for netBiDiB
//            2017-12-20       kw  introduced c99 check
//            2018-03-28 V0.25 kw  added query messages for drive states MSG_CS_QUERY, MSG_CS_DRIVE_STATE
//            2018-04-11       mos added load type BIDIB_PCFG_LOAD_TYPE for MSG_LC_CONFIGX
//            2018-12-29       kw  added __attribute__((__packed__))
//            2019-02-02       ab  extended FEATURE_GEN_RCPLUS_AVAILABLE into GEN_EXT_AVAILABLE
//                                 added defines for DCCA (MSG_CS_DCCA, MSG_CS_DCCA_ACK, Opcodes)
//            2019-02-10       ab  added MSG_BM_DCCA, updated DCCA opcodes
//            2019-03-04       kw  added new namespaces for DCCA (2)
//            2019-09-07       kw  added MSG_LOCAL_BIDIB_DOWN and MSG_LOCAL_BIDIB_UP
//            2019-11-23       kw  added FEATURE_STRING_DEBUG 
//            2020-05-07       kw  added FEATURE_GEN_REFERENCE and FEATURE_GEN_RF_CHANNEL -> renamed to FEATURE_CELL_NUMBER and FEATURE_RF_CHANNEL
//            2020-05-22       kw  changed: BIDIB_VERSION 0.7 to 0.8, added MSG_ACCESSORY_GETALL
//            2020-05-28       ab  added NODE_(UN)AVAILABLE for netBiDiB
//
//===============================================================================
//
...
#define MSG_DSYS                (MSG_DSTRM + 0x00)      // - // reserved - not used as msg_type
...
#define MSG_FEATURE_GETALL      (MSG_DFC + 0x00)        // [1:mode]
...
#define MSG_ACCESSORY_GETALL    (MSG_DACC + 0x04)       // -
...
#define MSG_CS_M4               (MSG_DGEN + 0x09)       // 1:opcode, [2..n:parameter]
#define MSG_CS_QUERY            (MSG_DGEN + 0x0A)       // 1:opcode (what (1=loco list), with MSB: all)
#define MSG_CS_DCCA             (MSG_DGEN + 0x0B)       // 1:opcode, [2..n:parameter]
...
#define MSG_LOCAL_LOGON_ACK     (MSG_DLOCAL + 0x00)     // 1:node_addr, 2..8:unique_id
#define MSG_LOGON_ACK           (MSG_DLOCAL + 0x00)     // deprecated name
#define MSG_LOCAL_PING          (MSG_DLOCAL + 0x01)
#define MSG_LOCAL_LOGON_REJECTED (MSG_DLOCAL + 0x02)    // 1..7:unique_id
#define MSG_LOGON_REJECTED      (MSG_DLOCAL + 0x02)     // deprecated name
#define MSG_LOCAL_ACCESSORY     (MSG_DLOCAL + 0x03)  //*// 1:statusflag, 2,3: DCC-accessory addr
#define MSG_LOCAL_SYNC          (MSG_DLOCAL + 0x04)  //*// 1:time_l, 2:time_h, [3..10: time utc]
#define MSG_LOCAL_DISCOVER      (MSG_DLOCAL + 0x05)  //*//
#define MSG_LOCAL_BIDIB_DOWN    (MSG_DLOCAL + 0x0C)     // 0x7C; serial mode wrapper, used for local queries
...
#define MSG_FEATURE_COUNT       (MSG_UFC + 0x02)        // 1:count, [2:mode]
...
#define MSG_BM_DYN_STATE        (MSG_UBM + 0x0a)        // 1:mnum, 2:addr_l, 3:addr_h, 4:dyn_num, 5..N:value
#define MSG_BM_RCPLUS           (MSG_UBM + 0x0b)        // 1:mnum, 2:opcode, [3..n:parameter]
#define MSG_BM_DCCA             (MSG_UBM + 0x0b)        // 1:mnum, 2:opcode, [3..n:parameter]
...
#define MSG_CS_M4_ACK           (MSG_UGEN + 0x09)       // 1:opcode, [2..n:parameter]
#define MSG_CS_DRIVE_STATE      (MSG_UGEN + 0x0A)       // 1:addrl, 2:addrh, 3:format, 4:active, 5:speed, 6:1-4, 7:5-12, 8:13-20, 9:21-28
#define MSG_CS_DCCA_ACK         (MSG_UGEN + 0x0B)       // 1:opcode, [2..n:parameter]
...
#define MSG_LOCAL_LOGON         (MSG_ULOCAL + 0x00)     // 1..7:unique_id
#define MSG_LOGON               (MSG_ULOCAL + 0x00)     // deprecated name
#define MSG_LOCAL_PONG          (MSG_ULOCAL + 0x01)     //
#define MSG_LOCAL_LOGOFF        (MSG_ULOCAL + 0x02)     // 1..7:unique_id
#define MSG_LOCAL_ANNOUNCE      (MSG_ULOCAL + 0x03)  //*// 1:opcode, 2..n:parameter
#define MSG_LOCAL_BIDIB_UP      (MSG_ULOCAL + 0x0C)     // 0xFC; serial mode wrapper, used for local queries

// special local messages
// that are sent without the distinction between up- and downstream
#define MSG_LOCAL_PROTOCOL_SIGNATURE     0xFE           // 1..5:'BiDiB', [6..n: debug]
#define MSG_LOCAL_LINK                   0xFF           // 1:opcode, 2..n:parameter

//===============================================================================
//
// 3. Type Defines
//    (useful defines for access to data structures in BiDiB)
//    These defines require C99 - thus we check c99
//
//===============================================================================
...
typedef struct __attribute__((__packed__))  // t_bidib_cs_accessory
  {
    union
      {
        struct
...
typedef struct __attribute__((__packed__))  // t_bidib_bin_state
  {
    union
      {
        struct
...
typedef struct __attribute__((__packed__))  // t_bidib_cs_pom
  {
    union
      {
        struct
...
typedef struct __attribute__((__packed__))  // t_bidib_cs_prog
  {
...
typedef struct __attribute__((__packed__))  // t_bidib_cs_prog_state
  {
    uint8_t result;                   //
    uint8_t time;                     //
    union
...
#define FEATURE_BST_CUTOUT_AVAILABLE       17   // booster can do cutout for railcom
...
#define FEATURE_GEN_RCPLUS_AVAILABLE       111  // (deprecated) 1: supports rcplus messages
#define FEATURE_GEN_EXT_AVAILABLE          111  // bitfield supported messages: 0 - RCPLUS, 1 - M4, 2 - DCCA, other: reserved
...
// 7.c) netBiDiB
// for MSG_LOCAL_LINK
#define BIDIB_LINK_DESCRIPTOR_PROD_STRING        0x00 // 2:size, 3..n:chars
#define BIDIB_LINK_DESCRIPTOR_USER_STRING        0x01 // 2:size, 3..n:chars
#define BIDIB_LINK_DESCRIPTOR_P_VERSION          0x80 // 2:proto-ver_l, 3:proto-ver_h
#define BIDIB_LINK_NODE_UNAVAILABLE              0x81 // 2:size, 3..m:chars, m+1: size, m+2..n:chars
#define BIDIB_LINK_NODE_AVAILABLE                0x82 // 
#define BIDIB_LINK_PAIRING_REQUEST               0xFC // 2..8:sender_uid, 9..15:receiver_uid
#define BIDIB_LINK_STATUS_UNPAIRED               0xFD // 2..8:sender_uid, 9..15:receiver_uid
#define BIDIB_LINK_STATUS_PAIRED                 0xFE // 2..8:sender_uid, 9..15:receiver_uid
#define BIDIB_LINK_DESCRIPTOR_UID                0xFF // 2..8:sender_uid
// for MSG_LOCAL_ANNOUNCE
#define BIDIB_ANNOUNCEMENT_SERVER_TCP_NODE       0x00 // 2:tcp_port_h, 3:tcp_port_l
...
#define BIDIB_CS_DRIVE_FORMAT_DCC128P    1  // SDF
#define BIDIB_CS_DRIVE_FORMAT_DCC28      2
#define BIDIB_CS_DRIVE_FORMAT_DCC128     3
#define BIDIB_CS_DRIVE_FORMAT_MM14       4
#define BIDIB_CS_DRIVE_FORMAT_MM27A      5
#define BIDIB_CS_DRIVE_FORMAT_MM27B      6
#define BIDIB_CS_DRIVE_FORMAT_M4         7  // MFX
...
// Accessory parameter
#define BIDIB_ACCESSORY_PARA_HAS_INFO  249   // ACC state will supply additional infos
...
#define BIDIB_PCFG_TICKS             0x0b      // uint8   pulse time for output [unit 10ms]
...
#define BIDIB_PCFG_LOAD_TYPE         0x0f      // uint8   type of load, for fault detection
// 16 bit values
...
#define BIDIB_PCFG_TRANSITION_TIME   0x46      // uint16  time for transition between port states [unit 10ms]
// 24 bit values
...
// DCCA Namesspaces (if not yet defined - see dcc_advanced.h)
#ifndef DCCA_SPACE_SHORTINFO
    #define DCCA_SPACE_SHORTINFO        0       // 5
    #define DCCA_SPACE_SHORTGUI         1       // 5
    #define DCCA_SPACE_FIRMWARE_ID      2       // 8
    #define DCCA_SPACE_PRODUCTNAME      3       // 8
    #define DCCA_SPACE_SHORTNAME        4
    #define DCCA_SPACE_FULLNAME         5
    #define DCCA_SPACE_FUNCMAP         11
    #define DCCA_SPACE_RAILCOMPAGE     12
    #define DCCA_SPACE_PICINFO         13
    #define DCCA_SPACE_CVp0            14
    #define DCCA_SPACE_CVp1            15
#endif

// a) opcodes for MSG_CS_DCCA (downstream)

#define BIDIB_DCCA_LOGON_ENABLE_ALL       0x00 // 2:count, 3:interval
#define BIDIB_DCCA_LOGON_ENABLE_LOCO      0x01 // 2:count, 3:interval
#define BIDIB_DCCA_LOGON_ENABLE_ACC       0x02 // 2:count, 3:interval
#define BIDIB_DCCA_LOGON_ENABLE_NOW       0x03 // 2:count, 3:interval

#define BIDIB_DCCA_LOGON_ASSIGN           0x08 // 2..5:dec_mun, 6:dec_mid, 7:new_addrl, 8:new_addrh
#define BIDIB_DCCA_GET_DATA               0x0F // 2:count (old: BIDIB_DCCA_GET_INFO_CONT)

#define BIDIB_DCCA_SELECT_INFO            0x10 // 2..5:addr/did, 6:mid, 7:namespace, 8..10:cv

#define BIDIB_DCCA_SELECT_INFO_SHORTINFO   (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_SHORTINFO     )  // size 5 - (note: to be keep in sequence with BIDIB_DCCA_GET_INFO)
#define BIDIB_DCCA_SELECT_INFO_SHORTGUI    (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_SHORTGUI      )  // size 5
#define BIDIB_DCCA_SELECT_INFO_FIRMWAREID  (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_FIRMWARE_ID   )  // size 8  manufacturer defined
#define BIDIB_DCCA_SELECT_INFO_PRODUCTNAME (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_PRODUCTNAME   )  // size 8  manufacturer defined
#define BIDIB_DCCA_SELECT_INFO_SHORTNAME   (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_SHORTNAME     )  // size 8  user
#define BIDIB_DCCA_SELECT_INFO_FULLNAME    (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_FULLNAME      )  // size 28 user
#define BIDIB_DCCA_SELECT_INFO_FUNCMAP     (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_FUNCMAP       )  // size var
#define BIDIB_DCCA_SELECT_INFO_RAILCOM     (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_RAILCOMPAGE   )  // size 256
#define BIDIB_DCCA_SELECT_INFO_PICTURE     (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_PICINFO       )  // -
#define BIDIB_DCCA_SELECT_INFO_CVp0        (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_CVp0          )  // 7:cv_l, 8:cv_h, 9:cv_x (note: command station performs toggle between phase 0 and 1)
#define BIDIB_DCCA_SELECT_INFO_CVp1        (BIDIB_DCCA_SELECT_INFO+DCCA_SPACE_CVp1          )  // 7:cv_l, 8:cv_h, 9:cv_x

// note: these are not yet implemented
#define BIDIB_DCCA_SET_INFO                0x20 // 2..5:addr/did, 6:mid, 7:namespace, 8..10:cv, 11..14:data
#define BIDIB_DCCA_SET_INFO_SHORTINFO      0x20 //
#define BIDIB_DCCA_SET_INFO_SHORTGUI       0x21 // -
#define BIDIB_DCCA_SET_INFO_SHORTNAME      0x24 // -
#define BIDIB_DCCA_SET_INFO_FULLNAME       0x25 // -
#define BIDIB_DCCA_SET_INFO_PICTURE        0x2D // -
#define BIDIB_DCCA_SET_INFO_CV             0x2E //

#define BIDIB_DCCA_GET_TID                0x30 // -
#define BIDIB_DCCA_SET_TID                0x31 // 2:cid[0],3:cid[1], 4:sid


// b) opcodes for MSG_CS_DCCA_ACK

#define BIDIB_DCCA_TID                    0x20 // 2:cid[0],3:cid[1], 4:sid
//      BIDIB_DCCA_GET_INFO*              0x1* // 2:ack, 3..6:addr/did, 7:mid
//      BIDIB_DCCA_LOGON_ENABLE_ALL       0x00 // 2:count, 3:interval
//      BIDIB_DCCA_LOGON_ENABLE_LOCO      0x01 // 2:count, 3:interval
//      BIDIB_DCCA_LOGON_ENABLE_ACC       0x02 // 2:count, 3:interval
//      BIDIB_DCCA_LOGON_ENABLE_NOW       0x03 // 2:count, 3:interval
//      BIDIB_DCCA_LOGON_ASSIGN           0x0E // 2:ack, 3..6:dec_mun, 7:dec_mid
//      BIDIB_DCCA_GET_INFO_CONT          0x0F // 2:ack

// c) opcodes for MSG_BM_DCCA
#define BIDIB_DCCA_LOGON_COLLISION        0x80 // -
#define BIDIB_DCCA_LOGON_NEW_DID          0x81 // 3:dec_mun[0],4:dec_mun[1],5:dec_mun[2],6:dec_mun[3], 7:dec_mid
#define BIDIB_DCCA_LOGON_ASSIGN_ACK       0x82 // 3:tbd, 4..7:hash
#define BIDIB_DCCA_INFO                   0x90 // 1..4:addr/did, 5:0/vid, [6..n:parameter]

#define BIDIB_DCCA_INFO_SHORTINFO          (BIDIB_DCCA_INFO+DCCA_SPACE_SHORTINFO            )  // 6:0, 7..11:shortinfo
#define BIDIB_DCCA_INFO_SHORTGUI           (BIDIB_DCCA_INFO+DCCA_SPACE_SHORTGUI             )  // 6:0, 7..11:shortgui
#define BIDIB_DCCA_INFO_FIRMWAREID         (BIDIB_DCCA_INFO+DCCA_SPACE_FIRMWARE_ID          )  // 6:index, 7..n:shortname
#define BIDIB_DCCA_INFO_PRODUCTNAME        (BIDIB_DCCA_INFO+DCCA_SPACE_PRODUCTNAME          )  // 6:index, 7..n:shortname
#define BIDIB_DCCA_INFO_SHORTNAME          (BIDIB_DCCA_INFO+DCCA_SPACE_SHORTNAME            )  // 6:index, 7..n:shortname
#define BIDIB_DCCA_INFO_FULLNAME           (BIDIB_DCCA_INFO+DCCA_SPACE_FULLNAME             )  // 6:index, 7..n:data
#define BIDIB_DCCA_INFO_FUNCMAP            (BIDIB_DCCA_INFO+DCCA_SPACE_FUNCMAP              )  // 6:index, 7..n:data
#define BIDIB_DCCA_INFO_RAILCOM            (BIDIB_DCCA_INFO+DCCA_SPACE_RAILCOMPAGE          )  // 6:index, 7..n:data
#define BIDIB_DCCA_INFO_PICTURE            (BIDIB_DCCA_INFO+DCCA_SPACE_PICINFO              )  // 6:index, 7..n:data
#define BIDIB_DCCA_INFO_CVp0               (BIDIB_DCCA_INFO+DCCA_SPACE_CVp0                 )  // 6:index, 7:cv_l, 8:cv_h, 9:cv_x, 10..n:data
#define BIDIB_DCCA_INFO_CVp1               (BIDIB_DCCA_INFO+DCCA_SPACE_CVp1                 )  // 6:index, 7:cv_l, 8:cv_h, 9:cv_x, 10..n:data
